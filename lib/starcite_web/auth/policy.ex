defmodule StarciteWeb.Auth.Policy do
  ...
  def can_create_session(%Context{kind: :jwt, tenant_id: tenant_id} = auth, _params)
  ...
  def can_list_sessions(%Context{kind: :jwt, tenant_id: tenant_id} = auth)
      when is_binary(tenant_id) and tenant_id != "" do
  ...
  def resolve_event_actor(%Context{kind: :jwt, subject: subject}, nil)
      when is_binary(subject) and subject != "" do
    {:ok, subject}
  end

  def resolve_event_actor(%Context{kind: :jwt, subject: subject}, requested_actor)
  ...
  defp session_permission(
         %Context{kind: :jwt, tenant_id: tenant_id} = auth,
  ...
  defp attach_principal_identity(metadata, %Context{
         kind: :jwt,
  ...
  defp attach_principal_identity(metadata, %Context{kind: :jwt, subject: subject})
       when is_binary(subject) and subject != "" do
