defmodule Starcite.Archive.Store do
  ...
  This module is intentionally persistence-only and does not normalize adapter
  failures. Adapter errors are returned as-is so callers can decide whether to
  propagate or crash.

  Runtime hot-cache behavior lives in `Starcite.Runtime.EventStore`.
  """
  ...
  def read_events(adapter_mod, session_id, from_seq, to_seq)
    ...
    adapter_mod.read_events(session_id, from_seq, to_seq)
  end
