defmodule Starcite.Runtime do
  ...
  def get_unarchived_events_for_archive_local(id, limit)
    ...
    with {:ok, server_id, lane, group} <- locate(id),
         :ok <- ensure_group_started(group) do
      case :ra.consistent_query(server_id, fn state ->
             {:ok, RaftFSM.query_unarchived(state, lane, id, limit)}
           end) do
        {:ok, {:ok, events}, _leader} when is_list(events) ->
          {:ok, events}
  ...
  @doc """
  List session IDs in one local group that still need archive catch-up.
  """
  @spec list_unarchived_sessions_by_group(non_neg_integer(), pos_integer()) ::
          {:ok, [String.t()]} | {:error, term()}
  def list_unarchived_sessions_by_group(group_id, limit)
      when is_integer(group_id) and group_id >= 0 and is_integer(limit) and limit > 0 do
    ...
    if Process.whereis(server_id) == nil do
      ...
      case :ra.consistent_query(server_id, fn state ->
             {:ok, RaftFSM.query_unarchived_sessions(state, limit)}
           end) do
        {:ok, {:ok, sessions}, _leader} when is_list(sessions) ->
          {:ok, sessions}

        {:ok, {{_term, _index}, {:ok, sessions}}, _leader} when is_list(sessions) ->
          {:ok, sessions}
  ...
  def list_unarchived_sessions_by_group(_group_id, _limit), do: {:error, :invalid_cursor}
