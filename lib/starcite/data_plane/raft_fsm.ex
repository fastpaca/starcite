defmodule Starcite.DataPlane.RaftFSM do
  ...
  def apply(meta, {:append_event, session_id, input, expected_seq}, state) do
    with {:ok, session} <- fetch_session(state.sessions, session_id),
      ...
      epoch = epoch_from_meta(meta, session)
      updated_session = put_session_epoch(updated_session, epoch)
      event_to_store = put_event_epoch(event_to_store, epoch)
      reply = decorate_append_reply(reply, epoch, updated_session.archived_seq)
  ...
  def apply(meta, {:append_events, session_id, inputs, opts}, state)
    ...
    with {:ok, session} <- fetch_session(state.sessions, session_id),
      ...
      epoch = epoch_from_meta(meta, session)
      updated_session = put_session_epoch(updated_session, epoch)
      replies = Enum.map(replies, &decorate_append_reply(&1, epoch, updated_session.archived_seq))
      events_to_store = put_events_epoch(events_to_store, epoch)
      ...
      reply =
        {:reply,
         {:ok,
          %{
            results: replies,
            last_seq: updated_session.last_seq,
            epoch: epoch,
            cursor: %{epoch: epoch, seq: updated_session.last_seq},
            committed_cursor: %{epoch: epoch, seq: updated_session.archived_seq}
          }}}
         ...
         {:ok,
          %{
            ...
            committed_cursor: %{
              ...
              seq: updated_session.archived_seq
            }
          }}}
  ...
  defp epoch_from_meta(%{term: term}, _session)
       when is_integer(term) and term >= 0,
       do: term

  defp epoch_from_meta(_meta, %Session{epoch: epoch})
       when is_integer(epoch) and epoch >= 0,
       do: epoch

  defp epoch_from_meta(_meta, _session), do: 0

  defp put_session_epoch(%Session{} = session, epoch)
