defmodule Starcite.ControlPlane.ReplicaRouter do
  ...
  defp route_target_with_meta(group_id, opts) when is_integer(group_id) and group_id >= 0 do
    ...
    case singleton_local_target(replicas, self_node, local_running, allow_local, prefer_leader) do
      {:ok, target, meta} ->
        {target, meta}

      :continue ->
        route_target_with_meta_full(
          group_id,
          opts,
          self_node,
          replicas,
  ...
  defp route_target_with_meta_full(
         group_id,
         opts,
         self_node,
         replicas,
       ...
       when is_integer(group_id) and group_id >= 0 and is_list(opts) and is_atom(self_node) and
              is_list(replicas) and is_boolean(local_running) and is_boolean(allow_local) do
    route_candidates =
    ...
    prefer_leader = Keyword.get(opts, :prefer_leader, false)
    defer_local_when_unconfirmed = Keyword.get(opts, :defer_local_when_unconfirmed, false)
  ...
  defp singleton_local_target(
       ...
       when is_atom(replica) and is_atom(self_node) and is_boolean(local_running) and
              is_boolean(allow_local) and
              is_boolean(prefer_leader) do
    cond do
      replica == self_node and local_running and allow_local ->
        meta = %{
          target: :local,
          prefer_leader: prefer_leader,
          leader_hint: if(prefer_leader, do: :singleton, else: :disabled),
          replica_count: 1,
          ready_count: 1
        }

        {:ok, {:local, self_node}, meta}
  ...
  defp remote_candidates_with_leader_first(
    ...
    if defer_local_when_unconfirmed and self_node not in leader_first do
      {local_ready, remote_ready} = Enum.split_with(remainder_ready, &(&1 == self_node))

      {local_fallbacks, remote_fallbacks} =
        Enum.split_with(remainder_fallbacks, &(&1 == self_node))

      Enum.uniq(
        leader_first ++ remote_ready ++ remote_fallbacks ++ local_ready ++ local_fallbacks
      )
    else
  ...
  defp cached_leader_hint(group_id, now_ms)
    ...
    case safe_cache_lookup(group_id) do
  ...
  defp put_leader_hint(group_id, leader_node, now_ms)
    ...
    _ = safe_cache_insert({group_id, leader_node, now_ms})
    :ok
  end

  defp safe_cache_lookup(group_id) when is_integer(group_id) and group_id >= 0 do
    try do
      :ets.lookup(@leader_cache_table, group_id)
    rescue
      ArgumentError ->
        # Table ownership can churn under high concurrency; recreate and retry once.
        ensure_leader_cache_table()
        safe_cache_lookup_retry(group_id)
    end
  end

  defp safe_cache_lookup_retry(group_id) when is_integer(group_id) and group_id >= 0 do
    try do
      :ets.lookup(@leader_cache_table, group_id)
    rescue
      ArgumentError -> []
  ...
  defp safe_cache_insert(entry) when is_tuple(entry) do
    try do
      ...
      ArgumentError ->
        # Table ownership can churn under high concurrency; recreate and retry once.
        ensure_leader_cache_table()
        safe_cache_insert_retry(entry)
    end
  end

  defp safe_cache_insert_retry(entry) when is_tuple(entry) do
    try do
      :ets.insert(@leader_cache_table, entry)
    rescue
      ArgumentError -> false
