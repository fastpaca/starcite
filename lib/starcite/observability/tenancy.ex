defmodule Starcite.Observability.Tenancy do
  ...
  @spec label_from_session(Session.t() | map()) :: String.t()
  ...
  @spec from_session(Session.t() | map()) :: String.t() | nil
  ...
  def from_session(%{metadata: metadata, creator_principal: creator_principal})
  ...
  defp principal_metadata_tenant_id(metadata) when is_map(metadata) do
    case Map.get(metadata, "starcite_principal") || Map.get(metadata, :starcite_principal) do
      principal_metadata when is_map(principal_metadata) ->
        metadata_tenant_id(principal_metadata)
