...
Fastpaca is a message backend for AI agents: an append-only, replayable conversation log with streaming updates and optional archival. It stores messages and sequence numbers. It does **not** build LLM prompts or manage token budgets.
...
const convo = await fastpaca.conversation('demo-chat', {
...
Most LLM apps need a durable, ordered message log and real-time updates. They also need flexible prompt assembly that changes as models, products, and policies evolve. Fastpaca handles the log; you control the prompt.
