...
- Phoenix `tail:<session_id>` topics subscribe before replaying, then receive in-process fanout updates; catch-up replay reads from the hot read path first and only falls back to Postgres when the requested range is not locally contiguous. If the fanout buffer lags, the server replays again to close the gap.
