...
- Appends now route through a local per-session worker boundary before they hit the repository layer. That gets append ordering off the HTTP task and onto one explicit session-local queue per process, with the worker owning the hot in-memory commit path before background Postgres flush catches durability up.
- The session worker now has a real local commit path: it updates hot in-memory session and event state, broadcasts live fanout, queues the event for a background Postgres flusher, and best-effort relays committed events directly to live peer nodes over the ops listener. Same-node reads can keep working while Postgres is briefly unavailable, and cross-node live delivery no longer has to wait for Postgres flush, but durable catch-up still trails the ack.
...
- `POST /v1/sessions/:id/append` now uses that owner hint directly. A wrong-node request is proxied to the current owner over plain HTTP instead of forcing the client to retry manually. If the owner cannot be reached, the edge returns `503 owner_proxy_unavailable` with the same owner hint.
...
- Raw `GET /v1/sessions/:id/tail` upgrades now return the same `409 session_not_owned` owner hints plus an additive `owner_tail_url`, preserving the current raw-tail query params for reconnectable browser clients.
...
- Committed lifecycle writes still fan out across Rust processes through Postgres `LISTEN/NOTIFY`, and flushed events continue to use that path as a repair/fallback signal. Session event commits now also best-effort relay directly to live peer nodes over the ops listener, so cross-node live delivery does not depend entirely on Postgres flush.
...
- Phoenix `tail:<session_id>` topics subscribe before replaying, then receive in-process fanout updates; catch-up replay reads from the hot read path first and only falls back to Postgres when the requested range is not locally contiguous. If the hot/cold merge still cannot produce a gap-free range, the server emits a `cursor_expired` gap instead of failing the socket. If the fanout buffer lags, the server replays again to close the gap.
...
already-open Phoenix topic subscription receives `{"type":"node_draining","reason":"node_draining","drain_source":"shutdown","retry_after_ms":N}`
...
The socket gauges reflect currently open Phoenix transports plus active Phoenix topic joins. The
Phoenix socket keeps reading frames continuously, so those gauges clear promptly when a quiet
client disconnects instead of waiting for the next broadcast.
